...
def process_image_pipeline(image_paths):
    print("Starting PaddleOCR pipeline...")
    ...
    # ArUco-based physical calibration
    from app.services.calibration_service import detect_aruco_scale

    pixels_per_mm = detect_aruco_scale(image_paths[0])

    if pixels_per_mm:
        print(
            f"ArUco calibration detected: "
    ...
    # Sort OCR results from top to bottom
    sorted_data = sorted(
        zoned_data,
        key=lambda item: sum(
            p[1] for p in item["bbox"]
        ) / 4.0
    )
    ...
    # Gemini is now used only for structured field extraction
    llm_extracted_data = extract_structured_data_llm(
        full_text
    )
